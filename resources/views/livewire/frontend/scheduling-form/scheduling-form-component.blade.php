    ...
    @else
        ...
        <div class="appointment-calendar bg-light p-4 rounded shadow-sm mb-4">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <button class="btn btn-outline-primary btn-sm"
                    wire:click="changeDays('prev')">Vorherige</button>
                <h5 class="m-0">
                    {{ $currentStartDate ? $currentStartDate->format("F Y") : "" }}
                </h5>
                <button class="btn btn-outline-primary btn-sm"
                    wire:click="changeDays('next')">Nächste</button>
            </div>
        ...
        <form wire:submit.prevent="submit" class="card shadow-sm p-4">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="selectedDate" wire:model="selectedDate"
            ...
            <div class="mb-3">
                ...
                <input type="email" wire:model="email" id="email" class="form-control"
                    placeholder="email@beispiel.com" required>
                @error("email")
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" wire:model="phone" id="phone" class="form-control"
                    placeholder="z.B. 0176-12345678" required>
                @error("phone")
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <!-- Services -->
            <div class="mb-3">
                <label for="services" class="form-label">Für welche Dienstleistung(en) interessieren
                    Sie sich?</label>
                <div>
                    <div class="form-check">
                        <input type="checkbox" wire:model="services"
                            value="Professionelle Webentwicklung" id="service1"
                            class="form-check-input">
                        <label for="service1" class="form-check-label">Professionelle
                            Webentwicklung</label>
                    </div>
                    <div class="form-check">
                        <input type="checkbox" wire:model="services"
                            value="Individuelle Softwarelösungen" id="service2"
                            class="form-check-input">
                        <label for="service2" class="form-check-label">Individuelle
                            Softwarelösungen</label>
                    </div>
                    <div class="form-check">
                        <input type="checkbox" wire:model="services"
                            value="Interaktive Benutzeroberflächen" id="service3"
                            class="form-check-input">
                        <label for="service3" class="form-check-label">Interaktive
                            Benutzeroberflächen</label>
                    </div>
                </div>
                @error("services")
                    <span class="text-danger">{{ $message }}</span>
