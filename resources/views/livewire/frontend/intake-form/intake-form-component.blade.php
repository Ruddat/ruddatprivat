<div class="container mt-5 col-md-6" data-aos="ease-in" data-aos-offset="120" data-aos-duration="1000" data-aos-delay="100" style="margin-top: 150px !important; margin-bottom: 100px;">
    ...
    @endif
        ...
        <div class="mb-3">
            ...
            <input type="text" id="name" class="form-control" wire:model="name" placeholder="Max Mustermann">
            @error('name') <span class="text-danger">{{ $message }}</span> @enderror
        </div>

        <div class="mb-3">
            <label for="email" class="form-label">E-Mail-Adresse *</label>
            <input type="email" id="email" class="form-control" wire:model="email" placeholder="email@beispiel.de">
            ...
            <div>
                @foreach(['Webentwicklung', 'Laravel-Entwicklung', 'CSS-Design', 'HTML5-Entwicklung', 'Bootstrap-Entwicklung', 'Java-Programmierung', 'AJAX-Entwicklung', 'MySQL-Datenbankverwaltung', 'Electron-Anwendungen'] as $service)
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" wire:model="services" value="{{ $service }}" id="service-{{ $loop->index }}">
        ...
        <div class="mb-3">
            <label for="projectDetails" class="form-label">Bitte beschreiben Sie Ihr Projekt im Detail</label>
            <textarea id="projectDetails" class="form-control" wire:model="projectDetails" rows="4"></textarea>
            @error('projectDetails') <span class="text-danger">{{ $message }}</span> @enderror
        </div>

        <div class="mb-3">
            <label for="referral" class="form-label">Wie sind Sie auf uns aufmerksam geworden?</label>
            ...
            @error('referral') <span class="text-danger">{{ $message }}</span> @enderror
        </div>

        <div class="mb-3">
            <label for="additionalComments" class="form-label">Weitere Fragen oder Anmerkungen</label>
            <textarea id="additionalComments" class="form-control" wire:model="additionalComments" rows="4"></textarea>
            @error('additionalComments') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
